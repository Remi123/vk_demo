...
struct logical_device {
  ...
  logical_device(const physical_device& phy_dev)
    ...
    std::set<uint32_t> uniqueQueueFamilies = {phy_dev.o_iQFam.value(), phy_dev.o_prQFam.value()};
    ...
    m_dev_ci.queueCreateInfoCount = static_cast<uint32_t>(queueCreateInfos.size());
    m_dev_ci.pQueueCreateInfos = queueCreateInfos.data();
    m_dev_ci.enabledExtensionCount = static_cast<uint32_t>(phy_dev.m_deviceExtensions.size());
    m_dev_ci.ppEnabledExtensionNames = phy_dev.m_deviceExtensions.data();

    if (vkCreateDevice(phy_dev.m_physicaldevice, &m_dev_ci, nullptr,
                       &m_dev) != VK_SUCCESS) {
